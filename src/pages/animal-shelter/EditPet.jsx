  ...
  useEffect(() => {
    const fetchPet = async () => {
      try {
        ...
        const response = await api.get(`/api/shelter/pets/${id}`);
        ...
        console.error('Fetch pet error:', error.response?.data || error);
        toast.error('Failed to load pet data: ' + (error.response?.data?.error || error.message));
  ...
  const handleSubmit = async (e) => {
    ...
    images.forEach((image, index) => {
      if (typeof image === 'object') {
        ...
        form.append(`existingImages[${index}]`, image);
      }
    });
    ...
    try {
      ...
      console.log('Submitting pet update:', Object.fromEntries(form));
      await api.put(`/api/shelter/pets/${id}`, form);
      toast.success('Pet updated successfully');
      navigate('/animalShelter-dashboard/adoptable-pets');
    } catch (error) {
      console.error('Update pet error:', error.response?.data || error);
      toast.error('Failed to update pet: ' + (error.response?.data?.error || error.message));
      if (error.response?.status === 401) {
        localStorage.removeItem('token');
        toast.error('Session expired. Please login again.');
        navigate('/auth/login');
      ...
      <div className="bg-white border-b border-neutral-200 shadow-sm">
        <div className="max-w-4xl px-4 mx-auto sm:px-6 lg:px-8">
          <div className="flex items-center py-6">
            ...
            <div>
              <h1 className="text-3xl font-bold text-neutral-900 font-display">Edit Pet</h1>
              <p className="mt-2 text-neutral-600">Update pet details for adoption</p>
            </div>
          </div>
        </div>
      </div>
