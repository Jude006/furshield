...
const FeaturedPets = () => {
    ...
    const fetchPets = async () => {
      try {
        ...
        const transformedPets = petsData.map(pet => ({
          ...pet,
          image: pet.image || 'https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&q=80&w=300'
        }));
  ...
  const clearFilters = useCallback(() => {
    ...
    // reset filters and search immediately
    setFilters({
      species: '',
      breed: '',
      age: '',
    ...
    setSearchQuery('');
  }, [debouncedHandleFilterChange, debouncedHandleSearchChange]);
  ...
  const filteredPets = useMemo(() => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value) {
        if (key === 'age') {
          result = result.filter(pet => pet.ageCategory.toLowerCase() === value.toLowerCase());
        } else {
          result = result.filter(pet => pet[key].toLowerCase() === value.toLowerCase());
        }
      }
    });
    return result;
  ...
  const viewPetDetails = useCallback((pet) => {
    setSelectedPet(pet);
    setShowModal(true);
  }, []);
  ...
  const handleAdoptClick = useCallback(() => {
    toast.success(`Adoption process started for ${selectedPet.name}!`);
    closeModal();
  }, [selectedPet, closeModal]);
  ...
  // Memoized PetCard component
  const PetCard = React.memo(({ pet, viewPetDetails }) => (
    <motion.div
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 border rounded-lg border-neutral-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              />
              {searchQuery && (
                <button
                  onClick={() => debouncedHandleSearchChange('')}
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => debouncedHandleFilterChange('species', e.target.value)}
                  ...
                  <div>
                    ...
                    <select
                      value={filters.breed}
                      onChange={(e) => debouncedHandleFilterChange('breed', e.target.value)}
                      className="w-full px-4 py-2 border rounded-lg border-neutral-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    >
                  ...
                  <div>
                    ...
                    <select
                      value={filters.age}
                      onChange={(e) => debouncedHandleFilterChange('age', e.target.value)}
                      className="w-full px-4 py-2 border rounded-lg border-neutral-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    >
                  ...
                  <div>
                    ...
                    <select
                      value={filters.size}
                      onChange={(e) => debouncedHandleFilterChange('size', e.target.value)}
                      className="w-full px-4 py-2 border rounded-lg border-neutral-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => debouncedHandleFilterChange('gender', e.target.value)}
                      className="w-full px-4 py-2 border rounded-lg border-neutral-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => debouncedHandleFilterChange('location', e.target.value)}
                      className="w-full px-4 py-2 border rounded-lg border-neutral-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    >
        ...
        {filteredPets.length === 0 ? (
          ...
          >
            ...
            {hasActiveFilters && (
              <button
                onClick={clearFilters}
                className="px-6 py-3 font-medium transition-colors duration-300 border rounded-lg text-primary-600 bg-primary-50 hover:bg-primary-100"
              >
                Show All Pets
              </button>
            )}
          ...
          <>
            ...
            >
              {filteredPets.map((pet) => (
                <PetCard key={pet.id} pet={pet} viewPetDetails={viewPetDetails} />
              ))}
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              ...
              <Link
                to={isAuthenticated ? '/pets-dashboard/adopt' : '/auth/register'}
              ...
              >
                ...
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="p-6 md:w-1/2">
                    ...
                    <div className="mt-6">
                      <Link
                        to={isAuthenticated ? '/pets-dashboard/adopt' : '/auth/signup'}
                        onClick={handleAdoptClick}
                        className="flex items-center justify-center px-6 py-3 font-medium text-white transition-colors duration-300 rounded-lg bg-primary-500 hover:bg-primary-600"
                      >
                        Adopt Now
                        <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3" />
