...
const ProductShowcase = () => {
      ...
      const fetchRatings = async () => {
        try {
          ...
          if (response.data.data.length > 0) {
            ...
            // Update products state to reflect new rating
            setProducts(prev => prev.map(p =>
              p._id === selectedProduct._id ? { ...p, rating: avgRating, reviewCount: response.data.data.length } : p
            ));
  ...
  const handleRatingSubmit = useCallback(async () => {
    ...
    try {
      ...
      setProductRatings(prev => [...prev, newRatingData]);
      setNewRating({ rating: 0, comment: '' });
      ...
      setProducts(prev => prev.map(p =>
        p._id === selectedProduct._id ? { ...p, rating: avgRating, reviewCount: p.reviewCount + 1 } : p
      ));
      setFilteredProducts(prev => prev.map(p =>
        p._id === selectedProduct._id ? { ...p, rating: avgRating, reviewCount: p.reviewCount + 1 } : p
      ));
  ...
  }, [selectedProduct, newRating, productRatings]);
